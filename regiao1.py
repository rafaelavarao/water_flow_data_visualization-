...
df1 = pd.read_csv('vazao4_2.csv', sep=";")

#Alterando a pontuação do valor de vazão
df1['value'] = df1['value'].astype(str).str.replace(',', '.').astype(float)

# Selecionando somente a quality=good e removendo a coluna quality, pois temos todos os valores iguais
df1['quality'] = df1['quality'].str.strip().str.lower()
# df1 = df1[df1['quality'] == 'good']
df1 = df1.rename(columns={'value': 'vazao'})
...
df1['day_of_week_name'] = df1['day_of_week'].map(dias_da_semana)
...
df_vazao['day_of_week'] = df_vazao['timestamp'].dt.dayofweek  # Segunda-feira: 0, Terça-feira: 1, ...

# Cria o gráfico de calendar heatmap
fig = px.density_heatmap(df_vazao, x="day_of_week_name", y="timestamp", z="vazao", color_continuous_scale="Viridis", title="Variação da Vazão ao Longo da Semana")
...
# # Traduzindo os nomes dos meses no primeiro conjunto de dados para o português
# translator = Translator()
# df_temperatura['Mês'] = df_temperatura['Mês'].apply(lambda x: translator.translate(x, dest='en').text)
